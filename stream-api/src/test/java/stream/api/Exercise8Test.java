...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void havingEnoughMoney() {
        ...
        Map<String, List<Item>> pricesOnSale = onSale.stream().collect(Collectors.groupingBy(Item::getName));
        Function<Item, Integer> getLowestPrice = item -> 
                pricesOnSale.getOrDefault(item.getName(), Collections.emptyList()).stream()
                .map(Item::getPrice)
        ...
        Predicate<Customer> havingEnoughMoney = customer -> customer.getBudget() > customer.getWantToBuy().stream()
                .map(getLowestPrice).reduce(Integer::sum).orElse(0);
